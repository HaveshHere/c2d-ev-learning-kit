...
void regenLedsAccelerate() {
  ...
  FastLED.show();
  // LED 1 red
  regenLeds[0] = CRGB::Green;   FastLED.show(); delay(200);
  // LED 2 red
  regenLeds[1] = CRGB::Green;   FastLED.show(); delay(200);
  // LED 3 green
  regenLeds[2] = CRGB::Red; FastLED.show(); delay(200);
  // LED 4 green
  regenLeds[3] = CRGB::Red; FastLED.show(); delay(200);
}
...
void runDrivingRegenFlow() {
  if (currentStep == 0) {
    setupRegenLeds();
    drawStartScreen("DRIVING & REGEN MODE", "Action: Press Accelerator Pedal");
    currentStep = 1;
